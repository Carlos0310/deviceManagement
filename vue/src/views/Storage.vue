<template>
  <div style="padding: 15px;width: 100%;overflow: scroll">
    ...
    <el-table :data="tableData" border style="width: 90%" stripe @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="storageId" label="记录编号" sortable min-width="110"/>
      <el-table-column prop="deviceName" label="设备名称" min-width="90"/>
      <el-table-column prop="placeName" label="设备处" min-width="90"/>
      <el-table-column prop="num" label="数量" min-width="70"/>
      <el-table-column prop="date" label="存放日期" min-width="120"/>
        ...
        <template #default="scope">
          <div style="display: flex;">
            ...
            <el-popconfirm
                ...
                icon-color="red"
                title="确认删除吗?"
                @confirm="handleDelete(scope.row.storageId)">
              <template #reference>
    ...
    <el-dialog
      ...
      <el-form :model="form" label-width="120px">
        <el-form-item label="记录编号" v-if="form.storageId">
          <el-input v-model="form.storageId" style="width: 80%" disabled></el-input>
        </el-form-item>
        ...
        <el-form-item label="存放日期">
          <el-date-picker v-model="form.date" value-format="YYYY-MM-DD" type="date" style="width: 80%"
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(val) {
      this.ids = val.map(v => v.storageId)   // [{id,name}, {id,name}] => [id,id]
    },
    deleteBatch() {
      if (!this.ids.length) {
      ...
      request.post("/storage/deleteBatch", this.ids).then(res => {
        if (res.code === '0') {
